...
const INSTRUCTIONAL_ACCEPTANCE =
...
function evaluationEvidenceStatus(row) {
  if (row.evaluation_evidence_payload?.declaredFixture === true) return "fixture";
  if (row.matching_eval_id == null && row.evaluation_evidence_id != null) return "stale";
  // Production trust is intentionally empty. The calibrator/evidence registry is the only
  // component allowed to promote a row to release-trusted.
  return "unverified";
}

function storedFindingGateInput(row) {
  const evidenceId = row.audio_evidence_id ?? null;
  return {
    analysisBasis: "acoustic",
    acousticDatasetManifestSha256: row.acoustic_dataset_manifest_sha256 ?? null,
    acousticDatasetVersion: row.acoustic_dataset_version ?? null,
    audioStatus: audioStatus(row.audio_retention ?? null, evidenceId !== null),
    ...
    evaluationEvidenceStatus: evaluationEvidenceStatus(row),
    evidenceId,
    modelArtifactSha256: row.model_artifact_sha256 ?? null,
    modelVersion: row.model_version_id ?? "",
    reviewStatus: row.review_status ?? "",
    sources: sortKeysDeep(row.source_refs ?? []),
...
export async function createTeacherReview(req, reply, ctx) {
  ...
  const body = await ctx.db.withTenant(actor.tenantId, async (tx) => {
    ...
    // Text-rule annotations say where a Quran rule applies. They contain no observation about how
    // this learner recited and therefore cannot be promoted into learner-performance feedback.
    // Reject before writing the review/audit or changing review_status, exactly as the Rust route.
    if (b.decision === "accepted" && finding.analysis_basis !== "acoustic") {
      throw new ApiError(INSTRUCTIONAL_ACCEPTANCE, 400);
    }
  ...
  const body = await ctx.db.withTenant(actor.tenantId, async (tx) => {
    const rows = await tx`
      ...
      LEFT JOIN eval_runs er
       ...
       AND er.calibrator_id = tf.calibrator_id
       AND er.calibrator_artifact_sha256 = tf.calibrator_artifact_sha256
      WHERE tf.tenant_id = ${actor.tenantId} AND tf.analysis_basis = 'acoustic'
-- ── What this queue is FOR ─────────────────────────────────────────────────────────────────
    ...
    const [{ total }] = await tx`
      ...
      WHERE tenant_id = ${actor.tenantId}
        AND analysis_basis = 'acoustic'
        AND review_status NOT IN ('teacher-reviewed', 'blocked', 'scholar-approved')`;
    totalAwaiting = total;

    return rows.map((r) => {
      const gateInput = storedFindingGateInput(r);
      return {
        acousticDatasetManifestSha256: gateInput.acousticDatasetManifestSha256,
        acousticDatasetVersion: gateInput.acousticDatasetVersion,
        analysisBasis: "acoustic",
        ...
        withheld: !clearsLearnerFeedbackGate(gateInput),
        wordId: r.word_id ?? "",
      };
    });
  });
