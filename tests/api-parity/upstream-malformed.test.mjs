...
test("findings with no acoustic semantics are rejected at the boundary", async () => {
...
test("a finding that is not an object cannot carry content to the learner", async () => {
  ...
  ml.respond = () => ({ status: 200, body: { annotations: [], findings: [smuggled, 42, null] } });
  const res = await predict();
...
test("a findings field that is not an array cannot carry content to the learner", async () => {
  ...
  ml.respond = () => ({ status: 200, body: { annotations: [], findings: { rule: smuggled, confidence: 0.99 } } });
  const res = await predict();
