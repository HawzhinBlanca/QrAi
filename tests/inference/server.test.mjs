...
const mlServer = await import("../../server/src/inference/runtime.mjs");
const { createCompatibilityIngress } = await import("../../server/src/inference/compatibility-ingress.mjs");
const { predictAlignment,
  transcribeSession,
  wavFromPcm16, predictTajweed, getAuditEvents, safeStorageSegment } = mlServer;
const compatibilityIngress = createCompatibilityIngress({
  audioObjectStore: { async put() {}, async get() {} },
  inference: {
    predictAlignment,
    predictTajweed,
    transcribeSession,
  },
  mlApiKey: "server-test-key",
  operationTimeoutMs: 1_000,
});

// Minimal IncomingMessage/ServerResponse pair for the worker compatibility handler. The audit GET
// never reads a body; the response captures the fixed JSON boundary.
function mockRequest(url, method = "GET") {
  return Object.assign(new EventEmitter(), {
    url,
    method,
    headers: { "x-ml-api-key": "server-test-key" },
...
test("alignment attribution is server-authored and digest-bound to the producing implementation", async () => {
  ...
  const expectedDigest =
    "sha256:" +
    createHash("sha256")
      .update(readFileSync(new URL("../../server/src/inference/alignment.mjs", import.meta.url)))
      .digest("hex");
...
test("the online ML service exposes no evaluation authority", () => {
  ...
  const source = readFileSync(new URL("../../server/src/inference/runtime.mjs", import.meta.url), "utf8");
...
// Exercise the worker compatibility dispatcher, not only the test-only audit accessor.
test("GET /v1/audit-events requires tenantId and never leaks another tenant's events", async () => {
  ...
  await compatibilityIngress(mockRequest("/v1/audit-events"), missing);
  assert.equal(missing.status, 400, "omitting tenantId must not expose every tenant's events");

  // With tenantId -> 200, scoped to exactly that tenant, tenant B's events absent.
  const res = mockResponse();
  await compatibilityIngress(mockRequest("/v1/audit-events?tenantId=audit-leak-tenant-a"), res);
...
function bootWith(env) {
  return new Promise((resolve) => {
    const child = spawnGuarded(process.execPath, [new URL("./lib/worker-compatibility-harness.mjs", import.meta.url).pathname], {
      env: {
...
test("the retention sweep destroys exactly what the contract says must be destroyed", async () => {
  ...
  const { retentionTtlHours } = await import("../../server/src/inference/runtime.mjs");
...
test("the retention sweep applies the same lifecycle to S3-shaped stores without filesystem paths", async () => {
  const now = Date.parse("2026-08-07T12:00:00.000Z");
  const objects = [
    ["discard-expired", "discard", now - 2 * 60 * 60 * 1000],
    ["review-current", "teacher-review", now - 2 * 60 * 60 * 1000],
    ["review-expired", "teacher-review", now - 8 * 24 * 60 * 60 * 1000],
    ["training", "training-opt-in", now - 365 * 24 * 60 * 60 * 1000],
    ["invalid-time", "discard", Number.NaN],
  ].map(([chunkId, audioRetention, storedAt]) => ({
    tenantId: "tenant-retention",
    learnerId: "learner-retention",
    sessionId: "session-retention",
    chunkId,
    objectKey: `audio/v1/tenant-retention/learner-retention/session-retention/${chunkId}.pcm`,
    audioRetention,
    storedAt: Number.isNaN(storedAt) ? "invalid" : new Date(storedAt).toISOString(),
  }));
  const deleted = [];
  const store = {
    async listAll() { return objects; },
    async get(object) { return { ...object, audioBytes: Buffer.from([0, 0]) }; },
    async deleteObject(object) { deleted.push(object.chunkId); },
  };
  const result = await mlServer.sweepExpiredAudio({ now, store });
  assert.deepEqual(deleted, ["discard-expired", "review-expired", "invalid-time"]);
  assert.deepEqual(result, { scannedCount: 5, deletedCount: 3, unreadableCount: 0 });
});
