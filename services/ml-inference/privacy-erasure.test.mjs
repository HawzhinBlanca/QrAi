 ...
 *
 *   node --test services/ml-inference/privacy-erasure.test.mjs
 *
 * ── Why this exists ─────────────────────────────────────────────────────────────────────────────
...
const ENTRY = join(here, "server.mjs");
...
test("a privacy delete removes the learner's audio from disk, and only theirs", async () => {
  ...
  assert.ok(
    Array.isArray(body.deletedMetadataObjectKeys) && body.deletedMetadataObjectKeys.length > 0,
    `the metadata is gone but the response reports no deleted metadata keys: ${JSON.stringify(body)}`,
  );
  for (const key of body.deletedAudioObjectKeys) {
    assert.ok(
      key.startsWith(`${TENANT}/${ERASED}/`),
...
test("GET /v1/audit-events is bounded, newest-first, and never truncates silently", async () => {
  ...
  const { clampAuditLimit } = await import("./server.mjs");
