...
- `services/platform-api`: Rust/Axum + SQLx/Postgres tenant-scoped API — auth (register/login,
  ...
  export/delete (with ML-service audio erasure and an `agent_runs` cascade), teacher reviews,
  scholar approvals, agent-run recording, eval-run lookup, audit events, and realtime ticket
  issuance. Tenant isolation enforced by Postgres RLS on every tenant-owned table.
...
- `infra/sql`: Postgres schema, full Quran seed, tenant RLS policies (every tenant-owned table),
  ...
  guard, per-tenant email uniqueness, and the `agent_runs.learner_id` erasure-support column.
