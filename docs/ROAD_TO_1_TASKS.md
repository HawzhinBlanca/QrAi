...
| **T12 — TLS + secure WS end-to-end** | ✅ **DONE** (proofs 2+3; proof 1's mic screenshot needs a real browser). Opt-in `docker-compose.tls.yml`: 80→301→443, HSTS, API+gateway proxied same-origin (`wss://`), backends unpublished, CSP `connect-src` tightened to `'self'`. Surfaced 3 real bugs: `VITE_*` were INERT (runtime env on a static nginx image; Vite inlines at build time) so `ws://127.0.0.1:8081` was baked in = mixed content → moved to `build.args`; the gateway rejects EVERY WS without `CORS_ALLOWED_ORIGINS` (fails closed); and the dormant `/ws/` `proxy_pass` lacks the load-bearing trailing slash (would 404). curl-proven on the running stack; `docker-build` green. | #234 |
| T6, T7, T8 | ⏳ ML, feasible in the same venv; T7/T8 gated on dataset licenses | — |
| T16, T18 | ⏳ straightforward-code, not started | — |
