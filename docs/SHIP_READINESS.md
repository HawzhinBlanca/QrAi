...
- **D9 — Secrets · 🧰 (one command → `scripts/gen-production-secrets.sh`).** The code **refuses
  ...
  (`ALLOW_INSECURE_SECRETS`, `ALLOW_SUPERUSER_DB_ROLE`, `METRICS_DEV_OPEN`,
  `GATEWAY_ALLOW_MISSING_ORIGIN`, `ALLOW_CHAOS_INJECTION` — see
  `specs/insecure-defaults-split/plan.md`). A **native/mobile client** needs only
...
- **F18 — i18n completeness · 🟡 infrastructure complete, English-only content.** `i18next` +
  ...
  extracted into `apps/web/src/locales/en.json` translation keys, and `App.tsx` drives
  `i18n.changeLanguage()` from the existing `activeLanguage` state so switching the language
  dropdown re-renders through i18next. The other 8 languages (`ckb`, `ar`, `tr`, `ur`, `id`, `ms`,
  `fr`, `de`) are registered with empty resource bundles and fall back to English
  (`fallbackLng: "en"`) — deliberately not fabricated, for the same reason
  `docs/SCHOLAR_REVIEW.md` requires native-speaker/scholar review before tajweed content ships:
  guessed translations for a religious-education product would be worse than honest English
  fallback. Canonical Quran text, tajweed rule content, and real backend/dynamic data (agent runs,
  teacher review notes, scholar approvals) are explicitly excluded from translation, each with an
  inline comment. Remaining work is a human decision (language prioritization + native-speaker
  review process) followed by populating the non-English resource files — the framework and
  extraction are done; the content is not.
