...
|----------|----------------|------------------------------------------|--------------------------|
| **Tajweed instruction / performance** | Server-owned inference runtime: deterministic `annotations[]` use `analysisBasis=text-rule`; acoustic `findings[]` require an approved producer | Instructional rules carry sources but no invented confidence/review state. Learner-performance findings are withheld unless `canShowLearnerFacingAiOutput` passes source, confidence, and teacher-review gates. | The deterministic rule scope remains scholar-gated; no text rule may be presented as measured learner pronunciation. Scholar scope approval = P3.6. |
