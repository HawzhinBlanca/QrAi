...
5. Run focused Node/Rust/Web tests, `git diff --check`, then the exact live-stack
   ...
   a missing declared test prerequisite, a non-portable derivation, or a newly published dependency
   advisory, preserve the affected gate: first reproduce the failure and map every evidence caller;
   prefer an already tracked byte-pinned source plus deterministic standard-library derivation over
