...
| `.github/workflows/ci.yml` minimum-Node suite projection | parses the one ordinary `scripts/verify.sh` Node test line using its `--test scripts/fixture-normalize` marker, then executes every extracted `.test.mjs` on Node 22.13.1 | preserve the parser marker while adding the W1.10 harness guard, assert the projection still finds the canonical line, and assert the extracted list contains the acoustic guard exactly once |
