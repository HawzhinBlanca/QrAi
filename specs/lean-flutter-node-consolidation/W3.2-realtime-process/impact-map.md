...
| `server/package.json` | standalone-lifecycle exact lint assertion; pnpm build/typecheck; Docker deploy | Include/pin the realtime entrypoint command without a dependency addition | exact package manifest, syntax/type/build, one production graph |
