...
| RPI-5 | WHEN discard, teacher-review, and training-opt-in sessions traverse the production image, THE proof SHALL verify claims-derived retention, immutable storage metadata, retained-only index/playback, actual retention cleanup, and privacy-safe evidence; IF Node, Postgres, or storage is interrupted, THEN fresh-ticket recovery or honest finalization SHALL close both captured-frame and transmitted-frame accounting, preserve genuine loss and unresolved client uncertainty as actionable, classify only observed stored-unindexed audio as repairable, keep restored repair idempotent, fail readiness closed while unsafe, and never report an incomplete recording complete. | retention/fault/recovery stages in the actual-image runner; storage-index, teacher-playback, retention, replay, and recovery regressions; evidence-stage contract test |
