...
| new actual-image probe/runner | release selection, Compose CLI, platform ticket/finalize routes, Node/Rust sockets, metrics, S3/Postgres, repair command | deterministic parity/hostile/retention/fault/load/soak against running immutable containers; the S3 runtime fault uses a bounded in-memory TLS-transparent pass-through owned by the proof runner, with an explicit pathless HTTPS target, preserved hostname/SNI, one `host-gateway` mapping, and a private RFC1918 Docker-interface bind—never a service or product dependency | command-plan unit tests, real byte-transparent TCP/cut proof, pass-through target/bind/port/identity mutations, bounded timeouts, restoration/cleanup, no raw data or secret output |
