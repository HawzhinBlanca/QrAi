...
| fault outcome/repair accounting | recovery controller; `realtime_audio_chunk_outcomes`; `repair-audio-index.mjs`; evidence validator and W3.9 reviewers | keep unsent client-dropped audio separate from durable server loss; repair only observed stored orphans; preserve genuinely absent accepted loss and unresolved client uncertainty as actionable, and never report their recording complete | exact closed equation, client/server outcome-class and uncertainty mutation tests, idempotent second pass, privacy-safe aggregate output |
