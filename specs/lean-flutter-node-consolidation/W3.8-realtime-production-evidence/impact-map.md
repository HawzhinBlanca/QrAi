...
| new actual-image probe/runner | release selection, Compose CLI, platform ticket/finalize routes, Node/Rust sockets, metrics, S3/Postgres, repair command | deterministic parity/hostile/retention/fault/load/soak against running immutable containers; `fault-recovery` binds all three fault lifecycles to one selection and one preflighted restricted-DB/production-S3 observation authority; the S3 runtime fault uses a bounded in-memory TLS-transparent pass-through owned by the proof runner, with an explicit pathless HTTPS target, preserved hostname/SNI, one `host-gateway` mapping, and a private RFC1918 Docker-interface bind—never a service or product dependency | command-plan/CLI dispatch tests, shared-authority cleanup/redaction, real byte-transparent TCP/cut proof, pass-through target/bind/port/identity mutations, bounded timeouts, restoration/cleanup, no raw data or secret output |
| fault outcome/repair accounting | recovery controller; proof-client acknowledgement delivery boundary; exact Node lifecycle; `realtime_audio_chunk_outcomes`; `repair-audio-index.mjs`; evidence validator and W3.9 reviewers | keep captured/transmitted totals and unsent client-dropped audio separate from durable server loss; establish sent/no-ack uncertainty without changing server behavior; repair only observed stored orphans; preserve genuinely absent accepted loss and unresolved client uncertainty as actionable, and never report their recording complete | real controller clean/ambiguous reports, exact capture/transmission equations, lifecycle/readiness/finalization/ticket mutations, idempotent repair second pass, privacy-safe aggregate output |
