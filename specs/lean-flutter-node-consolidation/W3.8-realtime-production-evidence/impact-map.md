...
| fault outcome/repair accounting | `realtime_audio_chunk_outcomes`; `repair-audio-index.mjs`; evidence validator and W3.9 reviewers | repair stored orphans only; preserve genuinely absent accepted loss as actionable and never report its recording complete | exact closed equation, outcome-class mutation tests, idempotent second pass, privacy-safe aggregate output |
